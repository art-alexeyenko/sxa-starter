...
module.exports = function generateConfig(configOverrides) {
  ...
  const xmc = xmCloud(config);
  const xmcConfig = Object.assign(config, xmc);
  // Handle undefined values
  Object.keys(xmcConfig).forEach((prop) => {
...
const config = {};\n`;
  ...
  Object.keys(computedConfig).forEach((prop) => {
    configText += `config.${prop} = process.env.VUE_APP_${constantCase(prop)} || ${computedConfig[
      prop
    ]?.trim()};\n`;
  });
